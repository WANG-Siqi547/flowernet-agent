...
_OVR_GENERATOR_PROVIDER_SET=${GENERATOR_PROVIDER+x}
_OVR_GENERATOR_PROVIDER_VAL=${GENERATOR_PROVIDER-}
_OVR_OUTLINER_PROVIDER_SET=${OUTLINER_PROVIDER+x}
_OVR_OUTLINER_PROVIDER_VAL=${OUTLINER_PROVIDER-}
_OVR_GENERATOR_PROVIDER_CHAIN_SET=${GENERATOR_PROVIDER_CHAIN+x}
_OVR_GENERATOR_PROVIDER_CHAIN_VAL=${GENERATOR_PROVIDER_CHAIN-}
_OVR_OUTLINER_PROVIDER_CHAIN_SET=${OUTLINER_PROVIDER_CHAIN+x}
_OVR_OUTLINER_PROVIDER_CHAIN_VAL=${OUTLINER_PROVIDER_CHAIN-}
_OVR_GENERATOR_MODEL_SET=${GENERATOR_MODEL+x}
_OVR_GENERATOR_MODEL_VAL=${GENERATOR_MODEL-}
_OVR_OUTLINER_MODEL_SET=${OUTLINER_MODEL+x}
_OVR_OUTLINER_MODEL_VAL=${OUTLINER_MODEL-}
_OVR_DEEPSEEK_MODEL_SET=${DEEPSEEK_MODEL+x}
_OVR_DEEPSEEK_MODEL_VAL=${DEEPSEEK_MODEL-}
_OVR_DEEPSEEK_BASE_URL_SET=${DEEPSEEK_BASE_URL+x}
_OVR_DEEPSEEK_BASE_URL_VAL=${DEEPSEEK_BASE_URL-}
_OVR_DEEPSEEK_API_URL_SET=${DEEPSEEK_API_URL+x}
_OVR_DEEPSEEK_API_URL_VAL=${DEEPSEEK_API_URL-}
_OVR_DEEPSEEK_API_KEY_SET=${DEEPSEEK_API_KEY+x}
_OVR_DEEPSEEK_API_KEY_VAL=${DEEPSEEK_API_KEY-}
...
if [ -n "${_OVR_GENERATOR_PROVIDER_SET:-}" ]; then
    GENERATOR_PROVIDER="$_OVR_GENERATOR_PROVIDER_VAL"
fi
if [ -n "${_OVR_OUTLINER_PROVIDER_SET:-}" ]; then
    OUTLINER_PROVIDER="$_OVR_OUTLINER_PROVIDER_VAL"
fi
if [ -n "${_OVR_GENERATOR_PROVIDER_CHAIN_SET:-}" ]; then
    GENERATOR_PROVIDER_CHAIN="$_OVR_GENERATOR_PROVIDER_CHAIN_VAL"
fi
if [ -n "${_OVR_OUTLINER_PROVIDER_CHAIN_SET:-}" ]; then
    OUTLINER_PROVIDER_CHAIN="$_OVR_OUTLINER_PROVIDER_CHAIN_VAL"
fi
if [ -n "${_OVR_GENERATOR_MODEL_SET:-}" ]; then
    GENERATOR_MODEL="$_OVR_GENERATOR_MODEL_VAL"
fi
if [ -n "${_OVR_OUTLINER_MODEL_SET:-}" ]; then
    OUTLINER_MODEL="$_OVR_OUTLINER_MODEL_VAL"
fi
if [ -n "${_OVR_DEEPSEEK_MODEL_SET:-}" ]; then
    DEEPSEEK_MODEL="$_OVR_DEEPSEEK_MODEL_VAL"
fi
if [ -n "${_OVR_DEEPSEEK_BASE_URL_SET:-}" ]; then
    DEEPSEEK_BASE_URL="$_OVR_DEEPSEEK_BASE_URL_VAL"
fi
if [ -n "${_OVR_DEEPSEEK_API_URL_SET:-}" ]; then
    DEEPSEEK_API_URL="$_OVR_DEEPSEEK_API_URL_VAL"
fi
if [ -n "${_OVR_DEEPSEEK_API_KEY_SET:-}" ]; then
    DEEPSEEK_API_KEY="$_OVR_DEEPSEEK_API_KEY_VAL"
fi
...
# 默认使用 SenseNova；如果配置了 DeepSeek key 且没有显式 provider，则优先 DeepSeek。
...
export GENERATOR_PROVIDER_CHAIN OUTLINER_PROVIDER_CHAIN

echo "✅ Provider 策略: GENERATOR_PROVIDER=$GENERATOR_PROVIDER, OUTLINER_PROVIDER=$OUTLINER_PROVIDER"
echo "✅ Provider Chain: GENERATOR_PROVIDER_CHAIN=${GENERATOR_PROVIDER_CHAIN:-<default>}, OUTLINER_PROVIDER_CHAIN=${OUTLINER_PROVIDER_CHAIN:-<default>}"
